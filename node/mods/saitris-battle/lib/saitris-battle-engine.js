...
function createInitialState({ sessionSeed, startTick, playerCount = 2 }) {
  let players = [];
  for (let i = 1; i <= playerCount; i++) {
    players.push(
      createPlayerState(
        hashStringToSeed(`${sessionSeed}-p${i}-${startTick}`),
        hashStringToSeed(`${sessionSeed}-g${i}-${startTick}`)
      )
    );
  }
...
class SaitrisBattleEngine {
  ...
  lockAndSpawn(playerIndex, pieceRng, garbageRng, currentTick) {
    ...
    if (cleared > 0) {
      if (this.isAuthoritativeFor(playerIndex)) {
        ...
        } else if (this.garbageMode === 'event' && this.onGarbage) {
          this.onGarbage({
  ...
  advanceToTick(targetTick) {
    while (this.lastProcessedTick < targetTick && !this.state.matchOver) {
      ...
      if (this.state.players.some((player) => player.knockouts >= 3)) {
  ...
  getStateChecksum() {
    return this.state.players
      .map(
        (p) =>
          `${p.knockouts},${p.linesSent},${p.pieceRngState},${p.garbageRngState},${p.active}`
      )
      .join('|');
  }

  applyStateSync(checksum) {
    checksum.split('|').forEach((data, i) => {
      let player = this.state.players[i];
      if (!player) {
        return;
      }
      let [knockouts, linesSent, pieceRngState, garbageRngState, active] = data.split(',');
      player.knockouts = parseInt(knockouts);
      player.linesSent = parseInt(linesSent);
      player.pieceRngState = parseInt(pieceRngState);
      player.garbageRngState = parseInt(garbageRngState);
      player.active = active === 'null' ? null : active;
    });
  }
